<x-dashboard-layout>
    ...
    <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <!-- Card Total Pesan -->
        <div
            class="bg-white shadow-xl rounded-2xl p-6 border-l-4 border-emerald-600 hover:shadow-2xl transition-all duration-300">
        ...
        <div
            class="bg-white shadow-xl rounded-2xl p-6 border-l-4 border-teal-600 hover:shadow-2xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-emerald-50 border-b border-emerald-100">
                        <th class="py-4 px-6 text-left text-sm font-semibold text-emerald-800 uppercase tracking-wider">
                            ID</th>
                        <th class="py-4 px-6 text-left text-sm font-semibold text-emerald-800 uppercase tracking-wider">
                            Nama</th>
                        <th class="py-4 px-6 text-left text-sm font-semibold text-emerald-800 uppercase tracking-wider">
                        ...
                        <th class="py-4 px-6 text-left text-sm font-semibold text-emerald-800 uppercase tracking-wider">
                            Pesan</th>
                        <th
                            class="py-4 px-6 text-center text-sm font-semibold text-emerald-800 uppercase tracking-wider">
                            Tanggal</th>
                        <th
                            class="py-4 px-6 text-center text-sm font-semibold text-emerald-800 uppercase tracking-wider">
                            Action</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach ($messages as $message)
                        <tr class="hover:bg-emerald-50 transition-colors duration-150">
                            ...
                            <td class="py-4 px-6 text-gray-700 text-center">{{ $message->created_at->format('d M Y') }}
                            </td>
                            <td class="py-4 px-6 text-gray-600 flex justify-center">
                                <form action="{{ route('admin.messages.destroy', $message->id) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="bg-red-500 hover:bg-red-600 text-white w-9 h-9 rounded-lg flex items-center justify-center transition-colors duration-200 shadow-md hover:shadow-lg">
